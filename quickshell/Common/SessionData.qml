...
Singleton {
    ...
    FileView {
        ...
        path: isGreeterMode ? "" : StandardPaths.writableLocation(StandardPaths.GenericStateLocation) + "/AtomShellos/session.json"
    ...
    Process {
        ...
        command: ["sh", "-c", "CONFIG_DIR=\"" + _stateDir
            + "/AtomShellos\"; if [ -f \"$CONFIG_DIR/default-session.json\" ] && [ ! -f \"$CONFIG_DIR/session.json\" ]; then cp --no-preserve=mode \"$CONFIG_DIR/default-session.json\" \"$CONFIG_DIR/session.json\" && echo 'copied'; else echo 'not_found'; fi"]
        running: false
        onExited: exitCode => {
            if (exitCode === 0) {
