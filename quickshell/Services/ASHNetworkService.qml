...
Singleton {
    ...
    function connectVpn(uuidOrName, singleActive = false) {
        ...
        DMSService.sendRequest("network.vpn.connect", params, response => {
            if (response.error) {
                ...
                pendingVpnUuid = ""
                vpnBusyStartTime = 0
                ToastService.showError("Failed to connect VPN")
            }
        })
    }
    ...
    function disconnectVpn(uuidOrName) {
        ...
        DMSService.sendRequest("network.vpn.disconnect", params, response => {
            if (response.error) {
                vpnIsBusy = false
                vpnBusyStartTime = 0
                ToastService.showError("Failed to disconnect VPN")
            }
        })
    }
    ...
    function disconnectAllVpns() {
        ...
        DMSService.sendRequest("network.vpn.disconnectAll", null, response => {
                if (response.error) {
                vpnIsBusy = false
                ToastService.showError("Failed to disconnect VPNs")
            }
        })
    }
    ...
    function setWifiAutoconnect(ssid, autoconnect) {
        ...
        DMSService.sendRequest("network.wifi.setAutoconnect", params, response => {
            if (response.error) {
                ToastService.showError("Failed to update autoconnect")
            } else {
                ToastService.showInfo(autoconnect ? "Autoconnect enabled" : "Autoconnect disabled")
                Qt.callLater(() => getState())
            }
        })
    }
