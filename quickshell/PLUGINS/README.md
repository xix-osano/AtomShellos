...
Browse and discover community plugins at **https://plugins.atomlinux.com/**

## Overview

Plugins let you add custom widgets to DankBar and Control Center. They're discovered from `~/.config/AtomShellos/plugins/` and managed via PluginService.
...
1. **PluginService** (`Services/PluginService.qml`)
   ...
   - Discovers plugins from `$CONFIGPATH/AtomShellos/plugins/`
...
Each plugin must be a directory in `$CONFIGPATH/AtomShellos/plugins/` containing:

```
$CONFIGPATH/AtomShellos/plugins/YourPlugin/
...
// Path to plugins directory ($CONFIGPATH/AtomShellos/plugins)
...
```bash
mkdir -p $CONFIGPATH/AtomShellos/plugins/MyPlugin
cd $CONFIGPATH/AtomShellos/plugins/MyPlugin
```
...
1. **Plugin Not Detected**
   ...
   - Verify directory is in `$CONFIGPATH/AtomShellos/plugins/`
...
4. **Data Not Persisting**
   ...
   - Check `$CONFIGPATH/AtomShellos/settings.json` for pluginSettings data
